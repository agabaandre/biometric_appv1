...
public class FingerLib {
  ...
  private int w_nLen;
  private int w_nBufOffset;

  public FingerLib(Activity parentActivity, TextView p_pStatusView) {
    m_bThreadWork = false;
    if (m_devComm == null) {
  ...
  public int SZOEMHost_Lib_Init(Activity parentActivity, TextView p_pStatusView) {
    m_bThreadWork = false;
        ...
        if (w_wPrefix == (short) (DevComm.CMD_PREFIX_CODE)) {
          ...
          if (m_dwCode != (short) (DevComm.CMD_FEATURE_OF_CAPTURED_FP_CODE)) {
            ...
            if ((m_devComm.m_nConnected == 1) || (m_devComm.m_nConnected == 3)) {
              ...
              w_blRet =
                  m_devComm.USB_ReceiveDataPacket((short) DevComm.CMD_FEATURE_OF_CAPTURED_FP_CODE);
            }
          }
        }
  ...
  public int Run_CmdUpImage() {
    ...
    new Thread(new Runnable() {
      ...
      public void run() {
        ...
        m_nImgWidth =
            (short) (((m_devComm.m_abyPacket[9] << 8) & 0x0000FF00) | (m_devComm.m_abyPacket[8]
                & 0x000000FF));
        m_nImgHeight =
            (short) (((m_devComm.m_abyPacket[11] << 8) & 0x0000FF00) | (m_devComm.m_abyPacket[10]
                & 0x000000FF));
            ...
            if (w_blRet == false) {
              ...
              if (m_devComm.GetRetCode() == DevComm.ERR_SUCCESS) {
                if (m_devComm.GetDataLen() > (DevComm.IMAGE_RECEIVE_UINT + 2)) {
                  ...
                  System.arraycopy(m_devComm.m_abyPacket, 8, m_binImage, m_nImageBufOffset,
                      m_devComm.GetDataLen() - 2);
